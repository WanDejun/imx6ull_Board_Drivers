...
uint8_t GT9147_CFG_TBL[] = {
    ,
};

gt9147_DevDefDtructure gt9147_dev;
int                    gt_init_fail = 0;
...
void gt9147_Init(void) {
    ...
    GPIO1->GDIR |= (1 << 9); /* GPIO1_IO09设置为输出 */
    GPIO1->DR |= (1 << 9);

    /* 复位IO初始化 */
    GPIO5->GDIR |= (1 << 9); /* GPIO1_IO09设置为输出 */
    GPIO5->DR |= (1 << 9);
    ...
    GPIO_WriteBit(GPIO5, GPIO_Pin_9, 0); /* 复位GT9147 */
    delay_ms(10);
    GPIO_WriteBit(GPIO5, GPIO_Pin_9, 1); /* 停止复位GT9147 */
    delay_ms(10);
    GPIO_WriteBit(GPIO1, GPIO_Pin_9, 0); /* 拉低INT引脚 */
    delay_ms(100);
    ...
    GPIO1->GDIR &= ~(1 << 9); /* GPIO1_IO09设置为输入 */
    GPIO1->DR &= ~(1 << 9);
    if (temp[0] != 0x31) {
    ...
    switch (temp[0]) {
        case 0x0:
            ...
            GPIO_IT_Config(GPIO1, GPIO_Pin_9, GPIO_IntRisingEdge);
            break;
        case 0x1:
            printf("InterruptMode:IntFallingEdge\r\n");
            GPIO_IT_Config(GPIO1, GPIO_Pin_9, GPIO_IntFallingEdge);
            break;
        case 0x2:
            printf("InterruptMode:IntLowLevel\r\n");
            GPIO_IT_Config(GPIO1, GPIO_Pin_9, GPIO_IntLowLevel);
            break;
        case 0x3:
            printf("InterruptMode:IntHighLevel\r\n");
            GPIO_IT_Config(GPIO1, GPIO_Pin_9, GPIO_IntHighLevel);
            break;
        default:
            printf("InterruptMode: Error\r\n");
            GPIO_IT_Config(GPIO1, GPIO_Pin_9, GPIO_IntBothEdge);
            break;
    }
    ...
    void GPIO1_Combined_0_15_IRQnHandler(void);
    system_register_irqhandler(GPIO1_Combined_0_15_IRQn, (system_irq_handler_t)GPIO1_Combined_0_15_IRQnHandler, NULL); /* 注册中断服务函数 */
    GPIO_IT_Enable(GPIO1, GPIO_Pin_9); /* 使能GPIO1_IO09的中断功能 */
...
void GPIO1_Combined_0_15_IRQnHandler(void) {
    ...
    GPIO_ClearITFlag(GPIO1, 9); /* 清除中断标志位 */
}
 ...
 * @return 		: 操作结果
 */
uint8_t gt9147_write_byte(uint8_t addr, uint32_t reg, uint8_t data) {
    uint8_t        status = 0;
    uint8_t        writedata = data;
    I2C_transfer_t masterXfer;

    /* 配置I2C xfer结构体 */
    masterXfer.slaveAddress = addr;   /* 设备地址 				*/
    masterXfer.direction = I2C_WRITE; /* 写入数据 				*/
 ...
 * @return 		: 读取到的数据。
 */
uint8_t gt9147_read_byte(uint8_t addr, uint32_t reg) {
    uint8_t val = 0;

    I2C_transfer_t masterXfer;
    masterXfer.slaveAddress = addr;  /* 设备地址 				*/
    masterXfer.direction = I2C_READ; /* 读取数据 				*/
 ...
 * @return 		: 无
 */
void gt9147_read_len(uint8_t addr, uint32_t reg, uint32_t len, uint8_t* buf) {
    I2C_transfer_t masterXfer;
 ...
 * @return 		: 无
 */
void gt9147_write_len(uint8_t addr, uint32_t reg, uint32_t len, uint8_t* buf) {
    I2C_transfer_t masterXfer;

    masterXfer.slaveAddress = addr;   /* 设备地址         */
    masterXfer.direction = I2C_WRITE; /* 读取数据 	    */
    masterXfer.subaddress = reg;      /* 要读取的寄存器地址 */
    masterXfer.subaddressSize = 2;    /* 地址长度一个字节     */
    masterXfer.data = buf;            /* 接收数据缓冲区 	    */
    masterXfer.dataSize = len;        /* 读取数据长度1个字节  */
