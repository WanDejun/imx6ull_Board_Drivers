...
/*GPIO翻转速度*/
typedef enum {
    GPIO_SPEED_0_low_50MHz = 0,
    GPIO_SPEED_1_medium_100MHz = 1,
    GPIO_SPEED_2_medium_100MHz = 2,
    GPIO_SPEED_3_max_200MHz = 3
} SPEED_t;

/*驱动能力(内阻大小)*/
typedef enum {
    // R0 = 260 Ohm(3_3V), 150 Ohm(1.8V), 240 Ohm(DDR)
    GPIO_DSE_0_output_driver_disabled = 0,
    GPIO_DSE_1_R0_1 = 1,
    GPIO_DSE_2_R0_2 = 2,
    GPIO_DSE_3_R0_3 = 3,
    GPIO_DSE_4_R0_4 = 4,
...
inline void GPIO_WriteBit(GPIO_Type* GPIO_Port, GPIO_Pin_t GPIO_Pin_x, BitAction_t BitAction);
inline void GPIO_WriteBits(GPIO_Type* GPIO_Port, unsigned int BitActions);
...
inline void GPIO_ClearITFlag(GPIO_Type* base, uint32_t GPIO_Pin_x); /*清除标志位*/
inline int  GPIO_GetITFlag(GPIO_Type* base, uint32_t GPIO_Pin_x); /*读取标志位*/
